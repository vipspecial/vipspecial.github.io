(function (global) {
  ...
  function shellMarkup() {
    return [
      ...
      '<div class="chat-history__head"><div><span>LEARNING JOURNAL</span><strong>学习记录</strong></div><button type="button" data-new-chat aria-label="新建对话">＋</button></div>',
      '<div class="chat-history__list" data-conversation-list><div class="history-loading">正在读取历史...</div></div>',
      '<div class="chat-history__foot"><i></i><span>学习记录已连接</span></div>',
      '</aside>',
      ...
      '</div>',
      '<p>AI 内容仅供学习参考，请核对重要信息 · 学习记录保存在匿名会话中</p>',
      '</form>',
      '</div>',
      '</section>'
    ].join('')
  ...
  function emptyMarkup() {
    return [
      ...
      '<span class="chat-empty__eyebrow">YOUR AI LEARNING COMPANION</span>',
      '<h1>今天，想学习什么？</h1>',
      '<p>从一个问题开始。理解知识、拆解难点，或为新的学习目标制定一份清晰计划。</p>',
      '<div class="chat-prompts">',
      '<button type="button" data-prompt="请用通俗的语言讲解一个我不理解的知识点，并举例说明">讲懂一个知识点 <i>↗</i></button>',
      '<button type="button" data-prompt="帮我制定一份目标明确、可以执行的学习计划">制定学习计划 <i>↗</i></button>',
      '<button type="button" data-prompt="帮我把一道复杂难题拆解成容易理解的步骤">拆解一道难题 <i>↗</i></button>',
      '</div>',
      '</div>'
    ].join('')
  }
  ...
  function messageMarkup(role, content, pending, reasoning) {
    ...
    return [
      ...
      '<div class="chat-message__body"><span>', role === 'user' ? '我的问题' : 'AI 学习助手', '</span>',
      reasoningMarkup,
